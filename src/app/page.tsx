...
function LiveFeed() {
  ...
  const items = [
    { icon: Signal,      label: 'Bin #0482 — Kashimira',     status: 'Overflow 82%',       tag: 'P1',    tagC: 'var(--error)',   tagBg: 'var(--error-container)' },
    { icon: Truck,       label: 'Truck #12 — Bhayandar East', status: 'Route Optimized',    tag: 'ETA 4m', tagC: 'var(--primary)', tagBg: 'var(--success-container)' },
    { icon: CheckCircle, label: 'Zone 7 — Mira Road',        status: 'Collection Complete', tag: '100%',   tagC: 'var(--primary)', tagBg: 'var(--success-container)' },
    { icon: Timer,       label: 'AI Engine',                  status: 'Recalculating...',    tag: '14ms',   tagC: 'var(--secondary)', tagBg: 'var(--info-container)' },
  ];

  useEffect(() => { const t = setInterval(() => setActive(p => (p + 1) % items.length), 2800); return () => clearInterval(t); }, []);

  return (
    <div className="space-y-2.5">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-bold uppercase" style={{ letterSpacing: '0.15em', color: 'var(--outline)' }}>Live Operations</span>
        <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[9px] font-bold uppercase" style={{ background: 'var(--error-container)', color: 'var(--error)', fontFamily: 'var(--font-mono)' }}>
          <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: 'var(--error)' }} /> Live
        </span>
      </div>
      {items.map((it, i) => {
        const Icon = it.icon;
        const lit = i === active;
        return (
          <motion.div key={i} animate={{ scale: lit ? 1.02 : 1, borderColor: lit ? 'rgba(0,193,106,0.25)' : 'transparent' }} className="flex items-center gap-3 p-3 rounded-xl border"
            style={{ background: lit ? 'rgba(0,193,106,0.04)' : 'rgba(255,255,255,0.4)', borderColor: 'transparent', transition: 'background .3s' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: lit ? 'var(--primary-container)' : 'var(--surface-high)' }}>
              <Icon size={15} color={lit ? 'white' : 'var(--outline)'} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-[10px] font-bold truncate" style={{ color: 'var(--outline)' }}>{it.label}</div>
              <div className="text-xs font-semibold truncate" style={{ fontFamily: 'var(--font-display)' }}>{it.status}</div>
            </div>
            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md flex-shrink-0" style={{ background: it.tagBg, color: it.tagC, fontFamily: 'var(--font-mono)' }}>{it.tag}</span>
          </motion.div>
        );
      })}
      {/* mini bar */}
      <div className="pt-2.5 mt-1" style={{ borderTop: '1px solid var(--outline-variant)' }}>
        <div className="flex justify-between items-center mb-1">
          <span className="text-[9px] font-bold uppercase" style={{ letterSpacing: '0.1em', color: 'var(--outline)' }}>System Load</span>
          <span className="text-[10px] font-bold" style={{ color: 'var(--primary)', fontFamily: 'var(--font-mono)' }}>Nominal</span>
        </div>
        <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--surface-high)' }}>
          <motion.div initial={{ width: 0 }} animate={{ width: '38%' }} transition={{ duration: 2.5, ease: [0.16, 1, 0.3, 1], delay: 0.5 }} className="h-full rounded-full" style={{ background: 'linear-gradient(90deg, var(--primary), var(--primary-container))' }} />
        </div>
      </div>
    </div>
  );
...
export default function LandingPage() {
  ...
  return (
    <div>
      ...
      <main>
        {/* ════════════════════════════════════════════════════
            §1  HERO — sticky, gradient mesh, massive type
           ════════════════════════════════════════════════════ */}
        <section
        ...
        >
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, rgba(0,193,106,0.08) 0%, rgba(57,184,253,0.06) 25%, rgba(255,136,66,0.04) 50%, rgba(0,109,57,0.06) 75%, rgba(57,184,253,0.08) 100%)',
          ...
          {/* Spotlight glow */}
          <div
            className="absolute w-[600px] h-[600px] rounded-full animate-spotlight opacity-30"
            style={{ background: 'radial-gradient(circle, rgba(0,193,106,0.2), transparent 70%)', top: '10%', left: '20%' }}
            aria-hidden="true"
          />

          {/* Dot grid */}
          <div className="absolute inset-0 opacity-[0.08]" style={{ backgroundImage: 'radial-gradient(circle, var(--primary) 0.5px, transparent 0.5px)', backgroundSize: '32px 32px' }} aria-hidden="true" />

          {/* Orbiting ring (decorative) */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] animate-orbit-slow pointer-events-none opacity-[0.06]" aria-hidden="true">
          ...
          <div className="max-w-[1400px] mx-auto px-6 md:px-10 w-full relative z-10 pt-24 pb-20">
            <div className="flex flex-col lg:flex-row items-center gap-12">
              ...
              >
                ...
                <motion.span
                  ...
                  style={{ background: 'rgba(0,193,106,0.08)', border: '1px solid rgba(0,193,106,0.12)', fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 600, color: 'var(--primary)' }}
                >
                  <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: 'var(--primary-container)' }} />
                  MIRA-BHAYANDAR MUNICIPAL CORP
                </motion.span>
                ...
                <motion.h1
                  ...
                  className="mb-5"
                  style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(40px, 5.5vw, 68px)', letterSpacing: '-0.04em', lineHeight: 1.02 }}
                >
                  Smart Waste.
                  <br />
                  <span className="animate-text-shimmer" style={{
                    background: 'linear-gradient(90deg, var(--primary-container), var(--secondary-container), var(--primary-container))',
                ...
                <motion.p
                  ...
                  className="mb-8"
                  style={{ fontSize: '15px', lineHeight: 1.75, color: 'var(--on-surface-variant)', maxWidth: '480px' }}
                >
                  Next-gen municipal infrastructure merging AI prediction, real-time IoT telemetry & route intelligence across 24 operational zones.
                </motion.p>
                ...
                >
                  <a href="#sensors" className="btn-primary gap-2 w-full sm:w-auto no-underline text-sm"><Map size={16} /> Explore City Map</a>
                  <a href="#analytics" className="btn-ghost w-full sm:w-auto no-underline text-sm">View Metrics</a>
                </motion.div>
              ...
              >
                <div className="animate-float">
                  <div
                    ...
                    style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(24px) saturate(180%)', boxShadow: '0 20px 60px rgba(0,0,0,0.06), 0 1px 3px rgba(0,0,0,0.04)' }}
                  >
                    {/* Glow accent */}
                    <div className="absolute -top-12 -right-12 w-40 h-40 rounded-full animate-scale-pulse" style={{ background: 'rgba(0,193,106,0.15)', filter: 'blur(40px)' }} aria-hidden="true" />
                    <LiveFeed />
                  </div>
                </div>
              </motion.div>
