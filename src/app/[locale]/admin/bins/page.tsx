          ...
          {filtered.map((bin, i) => {
            ...
            return (
              <motion.div
                ...
                className="p-4 border-b border-outline-variant/30 last:border-0 flex flex-col gap-3"
              >
                <div className="flex justify-between items-start">
                   <div className="flex items-center gap-2">
                     <Trash2 size={16} className="text-primary/70" />
                     <span className="font-bold text-sm" style={{ fontFamily: 'var(--font-mono)', color: 'var(--on-surface)' }}>{bin.serialNumber}</span>
                   </div>
                   <span
                    ...
                    >
                      {bin.sensorOnline ? <Wifi size={10} /> : <WifiOff size={10} />}
                      {statusChip.label}
                    </span>
                </div>
                
                <div className="flex items-center gap-1.5">
                  <MapPin size={14} style={{ color: 'var(--outline)' }} />
                  <span className="body-sm">{bin.zone}</span>
                  <span className="chip chip-neutral text-[10px] ml-1">{bin.ward}</span>
                </div>

                <div className="flex items-center justify-between gap-4 mt-2 bg-surface-lowest p-3 rounded-xl border border-outline-variant/20">
                   <div className="flex-1 flex flex-col gap-1.5">
                     <span className="text-[10px] font-bold text-outline uppercase tracking-wider">Fill Level</span>
                     <div className="flex items-center gap-2">
                       <div className="flex-1 h-1.5 rounded-full" style={{ background: 'var(--surface-high)' }}>
                         <div className="h-full rounded-full" style={{ width: `${bin.fillLevel}%`, background: getFillColor(bin.fillLevel), boxShadow: bin.fillLevel >= 80 ? '0 0 8px rgba(186,26,26,0.3)' : 'none' }} />
                       </div>
                       <span className="font-mono text-[11px] font-bold" style={{ color: getFillColor(bin.fillLevel) }}>{bin.fillLevel}%</span>
                     </div>
                   </div>
                   
                   <div className="flex flex-col gap-1.5 items-end pl-4 border-l border-outline-variant/30">
                     <span className="text-[10px] font-bold text-outline uppercase tracking-wider">Battery</span>
                     <div className="flex items-center gap-1 text-[11px] font-mono">
                       {bin.batteryLevel <= 20 ? <BatteryLow size={12} className="text-error" /> : <Battery size={12} className="text-outline" />}
                       <span style={{ color: bin.batteryLevel <= 20 ? 'var(--error)' : 'var(--on-surface-variant)' }}>{bin.batteryLevel}%</span>
                     </div>
                   </div>
                </div>

                <div className="flex justify-between items-center mt-1 pt-3 border-t border-outline-variant/20">
                  <span className="text-[10px] font-mono text-outline flex items-center gap-1">
                    <Clock size={10} /> {bin.lastPing}
                  </span>
                  <div className="flex gap-2">
                     <button className="p-1.5 rounded-lg bg-surface-low text-outline hover:text-primary transition-colors">
                       <Eye size={14} />
                     </button>
                     <button className="p-1.5 rounded-lg bg-surface-low text-outline hover:text-primary transition-colors">
